...
rf.model = h2o.randomForest(x = c(1:7091),
                            y = 7092,
...
# variable importance
var.imp = data.frame(h2o.varimp(rf.model))

# confusion matrix
h2o.confusionMatrix(rf.model)

# predictions
preds =h2o.predict(object = rf.model, newdata = test.hex)
preds_raw = as.numeric(as.character(as.data.frame(preds$predict)$predict))

sum(preds_raw==y3_test_dum)/length(y3_test_dum)

#######################################################################
### RANDOM FOREST MODEL: LAG + RESTAURANT DUMMIES
#######################################################################
...
############################################################################
### RANDOM FOREST MODEL: LAG + RESTAURANT_DUMMIES + RESTAURANT_CATEGORIES
############################################################################

rf.model_3 = h2o.randomForest(x = c(5001:7091),
                              y = 7092,
                              training_frame = train.hex,
                              #classification = T,
                              ntree = NTREES,
                              ...
                              #nfolds = 3,
                              validation = test.hex)
sink('model_diagnostics/m_3_diagnostics.txt')
rf.model_3

# variable importance
var.imp_3 = data.frame(h2o.varimp(rf.model_3))
print('**************************************')
print('MODEL_3')
print('**************************************')
print('VARIABLE IMPORTANCE')
print(var.imp_3[1:30,])

# confusion matrix
print('CONFUSION MATRIX')
h2o.confusionMatrix(rf.model_3)

# predictions
preds_3 = as.data.frame(h2o.predict(object = rf.model_3, newdata = test.hex))
preds_raw_3 = preds_3$predict
preds_raw_3_2 = ifelse(preds_3$p1 > 0.5, 1, 0)

# accuracy
print('ACCURACY (second one is with p = 0.5')
sum(preds_raw_3==y3_test_dum)/length(y3_test_dum)
sum(preds_raw_3_2==y3_test_dum)/length(y3_test_dum)

#auc
print('AUC')
print(auc(y3_test_dum,preds_3$p1))

# diagnostic plot
png(file='model_diagnostics/m_3_roc1.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
plot(roc(y3_test_dum,preds_3$p1), xlim=c(1,0))
dev.off()

png(file='model_diagnostics/m_3_pred_dist.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
plot_pred_type_distribution(data.frame(pred=preds_3$p1,
                                       actual=y3_test_dum), 0.5)
dev.off()


png(file='model_diagnostics/m_3_roc2.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
roc <- calculate_roc(data.frame(pred=preds_3$p1,
                                actual=y3_test_dum), 1, 1, n = 100)
plot_roc(roc, 0.4, 1, 1)
dev.off()

#PRECISION-RECALL
png(file='model_diagnostics/m_3_prec-recall.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
pred2 = prediction(preds_3$p1,y3_test_dum)
RP.perf <- performance(pred2, "prec", "rec")
plot (RP.perf)
dev.off()

png(file='model_diagnostics/m_3_roc3.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
ROC.perf <- performance(pred2, "tpr", "fpr")
plot (ROC.perf)
lines(seq(0,1,0.001), seq(0,1,0.001), lty=2)
...
# F1
print('PRECISION')
precision <- posPredValue(as.factor(preds_raw_3_2), as.factor(y3_test_dum))
precision

print('RECALL')
recall <- sensitivity(as.factor(preds_raw_3_2), as.factor(y3_test_dum))
recall
...
sink()
# save model
h2o.saveModel(rf.model_3, path = "h2o_models/m_3", force = FALSE)
...
sum(preds_raw==y3_test_dum)/length(y3_test_dum)
sum(preds_raw2==y3_test_dum)/length(y3_test_dum)
sum(preds_raw3==y3_test_dum)/length(y3_test_dum)


# AVERAGE OF 3 MODELS

temp = cbind(preds$p1, preds2, preds3)
preds4 = rowMeans(temp)
preds_raw4 = ifelse(preds_raw4 > .5, 1, 0)

sum(preds_raw4==y3_test_dum)/length(y3_test_dum)
...
rf.model_4 = h2o.randomForest(x = c(which.cols),
                             y = 7092,
                             training_frame = train.hex,
                             #classification = T,
                             ntree = NTREES,
                             ...
                             #nfolds = 3,
                             validation = test.hex)
sink('model_diagnostics/m_4_diagnostics.txt')
rf.model_4

# variable importance
var.imp_4 = data.frame(h2o.varimp(rf.model_4))
print('**************************************')
print('MODEL_4')
print('**************************************')
print('VARIABLE IMPORTANCE')
print(var.imp_4[1:30,])

# confusion matrix
print('CONFUSION MATRIX')
h2o.confusionMatrix(rf.model_4)

# predictions
preds_4 = as.data.frame(h2o.predict(object = rf.model_4, newdata = test.hex))
preds_raw_4 = preds_4$predict
preds_raw_4_2 = ifelse(preds_4$p1 > 0.5, 1, 0)

# accuracy
print('ACCURACY (second one is with p = 0.5')
sum(preds_raw_4==y3_test_dum)/length(y3_test_dum)
sum(preds_raw_4_2==y3_test_dum)/length(y3_test_dum)

#auc
print('AUC')
print(auc(y3_test_dum,preds_4$p1))

# diagnostic plot
png(file='model_diagnostics/m_4_roc1.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
plot(roc(y3_test_dum,preds_4$p1), xlim=c(1,0))
dev.off()

png(file='model_diagnostics/m_4_pred_dist.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
plot_pred_type_distribution(data.frame(pred=preds_4$p1,
                                       actual=y3_test_dum), 0.5)
dev.off()


png(file='model_diagnostics/m_4_roc2.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
roc <- calculate_roc(data.frame(pred=preds_4$p1,
                                actual=y3_test_dum), 1, 1, n = 100)
plot_roc(roc, 0.4, 1, 1)
dev.off()

#PRECISION-RECALL
png(file='model_diagnostics/m_4_prec-recall.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
pred2 = prediction(preds_4$p1,y3_test_dum)
RP.perf <- performance(pred2, "prec", "rec")
plot (RP.perf)
dev.off()

png(file='model_diagnostics/m_4_roc3.png', width=5,height=5, units='in',
    type = 'cairo', res=200)
ROC.perf <- performance(pred2, "tpr", "fpr")
plot (ROC.perf)
lines(seq(0,1,0.001), seq(0,1,0.001), lty=2)
...
# F1
print('PRECISION')
precision <- posPredValue(as.factor(preds_raw_4_2), as.factor(y3_test_dum))
precision

print('RECALL')
recall <- sensitivity(as.factor(preds_raw_4_2), as.factor(y3_test_dum))
recall
...
sink()
# save model
h2o.saveModel(rf.model_4, path = "h2o_models/m_4", force = FALSE)

#rf.model_4 = h2o.loadModel(path = 'h2o_models/m4/DRF_model_R_1465466450879_8/')
...
temp = cbind(preds_2$p1,preds_3$p1,preds_4$p1)
preds5 = rowMeans(temp)
preds_raw5 = ifelse(preds5 > .5, 1, 0)

sum(preds_raw5==y3_test_dum)/length(y3_test_dum)
...
## OLD ##
################################################
rf.varImportance = rf.model@model$varimp
options(scipen=999)
print(rf.varImportance)

topVars = which(rf.varImportance[1,] > 50 )
topVars2 = rf.varImportance[1,topVars]

topVars2[,order(topVars2)]
trainingFrame = as.data.frame(trainingMat)
head(trainingFrame)

mean(y1[trainingFrame$bread.pudding>0])
mean(y1[trainingFrame$bread.pudding==0])

mean(y1[trainingFrame$drinks>0])
mean(y1[trainingFrame$drinks==0])

mean(y1[trainingFrame$rosemary>0])
mean(y1[trainingFrame$rosemary==0])

mean(y1[trainingFrame$scallion.pancakes>0])
mean(y1[trainingFrame$scallion.pancakes==0])


aggregate(y1 ~ trainingFrame$noodle,FUN = mean)

plot(y1 ~ trainingFrame$noodle)
plot(y1 ~ trainingFrame$scallion.pancakes, pch=19)
